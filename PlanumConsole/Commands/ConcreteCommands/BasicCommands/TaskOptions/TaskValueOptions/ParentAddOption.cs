...
namespace Planum.Console.Commands.Task
{
    public class ParentAddOption : BaseOption<TaskCommandSettings>
    {
        ...
        public ParentAddOption(ILoggerWrapper logger, TaskBufferManager taskBufferManager, OptionInfo optionInfo, ConsoleConfig commandConfig) : base(logger, optionInfo, commandConfig)
        {
        ...
        public override bool TryParseValue(ref IEnumerator<string> args, ref List<string> lines, ref TaskCommandSettings result)
        {
            ...
            else
            {
                foreach (var task in result.Tasks)
                    task.Parents = task.Parents.Concat(parents).ToHashSet();
            }
            return true;
        }
    }
