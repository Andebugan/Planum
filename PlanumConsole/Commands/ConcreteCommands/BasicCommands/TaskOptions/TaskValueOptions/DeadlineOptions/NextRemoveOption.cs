...
namespace Planum.Console.Commands.Task
{
    public class NextRemoveOption: BaseOption<TaskCommandSettings>
    {
        ...
        public NextRemoveOption(ILoggerWrapper logger, TaskBufferManager taskBufferManager, OptionInfo optionInfo, ConsoleConfig commandConfig): base(logger, optionInfo, commandConfig)
        {
            ...
            else
            {
                foreach (var task in result.Tasks)
                {
                    ...
                    foreach (var deadline in filteredDeadlines)
                        deadline.next = deadline.next.Except(next).ToHashSet();
